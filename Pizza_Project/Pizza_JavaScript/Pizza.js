...
function getTopping(runningTotal,text1) {
    ...
    for (var k = 0; k < vegetableArray.length; k++) {
        if (vegetableArray[k].checked) {
            ...
            text1 += vegetableArray[k].value + "<br>";
        }
    }
    ...
    if (toppingCount + vegetableCount > 1) {
        toppingTotal = (toppingCount + vegetableCount - 1);
    } else {
        toppingTotal = 0;
    }
    runningTotal = (runningTotal + toppingTotal);
    console.log("Total Selected topping items: "+toppingCount);
    console.log(vegetableCount+toppingCount+" topping - 1 free topping = "+"$"+toppingTotal+".00");
